...
#### Random Forest
To add reference
The random forest plot is an extension of the decision tree methodology to reduce the variance. Decision trees are very sensitive to the training data set and can lead to high variance; thus potential issues with generalisation of the model. The random forest plot selects random observation of the data set to create multiple decision trees. Random variables are selected for each tree to use in the training of the data set. The aggregated output of the generated decision trees is then used to create an estimate.


#### Gradient Boosting
Gradient boosting is a machine learning algorithm that uses decision tree a a base model. The data is initially trained
on this decision tree, and initial prediction is weak, thus termed a weak based model. In gradient boosting the process
is iterative, thus further decision tree is sequentially added, each tree learns from the prior tree(s) to each model to
give strength to the model and reduce errror.
...
support vectors are the only data points that have an influence of the maximum margin in SVM.
