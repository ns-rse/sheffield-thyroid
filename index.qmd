...
#### Gradient Boosting
Gradient boosting is a machine learning algorithm that uses decision tree a a base model. The data is initially trained on this decision tree, and initial prediction is weak, thus termed a weak based model. In gradient boosting the process is iterative, thus further decision tree is sequentially added, each tree learns from the prior tree(s) to each model to give strength to the model and reduce errror.
