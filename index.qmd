...
binary_logistics_clin <- glm(final_pathology ~ age_at_scan + gender + incidental_nodule + palpable_nodule + rapid_enlargment + compressive_symtoms + hashimotos_thyroiditis + family_history_thyroid_cancer + exposure_radiation + tsh_value + size_nodule_mm + solitary_nodule + cervical_lymphadenopathy, data = df, family = binomial(link = "logit"))
summary(binary_logistics_clin)
coefficients_clin <- coef(binary_logistics_clin)
odds_ratios_clin <- exp(coefficients)
print(odds_ratios_clin)
conf_interval <- exp(confint(binary_logistics_clin))
results <- cbind(OddsRatio = odds_ratios_clin, conf_interval)
print(results)
```
...
summary(binary_logistic_biochem)
```
...
summary(binary_logistic_ultrasound)
```
...
```{r}
binary_logistic_vars <- glm(final_pathology ~ age_at_scan + gender + incidental_nodule + tsh_value + size_nodule_mm + solitary_nodule + cervical_lymphadenopathy + bta_u_classification + cervical_lymphadenopathy + thy_classification, data = df, family = binomial)
summary(binary_logistic_ultrasound)
```
...
binary_logistic_model <- logistic_reg() |>
  set_engine("glm")
## add the binary logistic regression model to the thyroid workflow
binary_log_thyroid_workflow <- thyroid_workflow |>
  add_model(binary_logistic_model)
## fit the the workflow to the training data
binary_log_thyroid_fit <- fit(binary_log_thyroid_workflow, data = train)
## to inspect the fit object
str(binary_log_thyroid_fit)
## use fitted model to make prediction 
binary_logistic_model_predictions <- predict(binary_log_thyroid_fit, test) |> bind_cols(test)
## examine the processing steps
binary_log_thyroid_fit |> extract_recipe()
## examine the model
binary_log_thyroid_fit |> extract_fit_parsnip()
## to check if the workflow has been trained
binary_log_thyroid_fit$trained
...
#### Gradient Boosting
{{< include sections/gradientboost.qmd >}}
