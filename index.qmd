...
binary_logistics_clin <- glm(final_pathology ~ age_at_scan + gender + incidental_nodule + palpable_nodule + rapid_enlargment + compressive_symtoms + hypertension + graves_disease + hashimotos_thyroiditis + family_history_thyroid_cancer + exposure_radiation, data = df, family = binomial)
...
extracted_recipe <- binary_log_thyroid_fit |> extract_recipe()
print(extracted_recipe)
str(train)
print(binary_logistic_model)
print(thyroid_workflow)
extracted_fit <- binary_log_thyroid_fit |> extract_fit_parsnip()
