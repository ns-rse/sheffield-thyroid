...
<<<<<<< HEAD
Cross validation was used to estimate the accuracy of the various machine learning models.
The k-fold techniques was used to splits the data in ?10 folds, and the data trained on all but one of the the fold; the single fold not trained is then and the
used to test the data. This was repeated multiple times using a different fold for test and
the others folds for training until all the folds is utilised for training and testing. Following multiple training process
with k-fold, we selected the model that has the best predictive value for thyroid cancer in the test cohort. 
We also used the leave one out (loo) cross-validation to train and test the datasets. In this technique, all but one
observation is use to train the data set and one observation is use to test the data; this was repeated until all the
=======
...
>>>>>>> 41e9babebd30a0c96c2522d73a479db4a7f6d1a0
